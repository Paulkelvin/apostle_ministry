...
export function EventDetailModal({ event, onClose }: EventDetailModalProps) {
  ...
  const getCategoryColor = (cat?: string) => {
    ...
    return cat ? colors[cat] || 'bg-warm-100 text-warm-600' : 'bg-warm-100 text-warm-600'
  }
      ...
      {event && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-3xl max-h-[90vh] bg-white rounded-xl shadow-[--shadow-card-hover] overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close Button */}
            <button
              onClick={onClose}
              className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center text-warm-500 hover:text-warm-900 hover:bg-white transition-all"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="overflow-y-auto flex-1">
                ...
                <div className={`p-6 md:p-8 ${event.image ? 'md:w-1/2' : 'w-full'}`}>
                  ...
                  <div className="mt-5 space-y-3">
                    <div className="flex items-start gap-3 text-warm-600">
                      ...
                      <div>
                        <p className="font-semibold text-warm-900">{format(parseISO(event.date), 'EEEE, MMMM d, yyyy')}</p>
                        {event.endDate && (
              ...
              {/* Bottom Action Bar */}
              <div className="sticky bottom-0 bg-white border-t border-warm-200 px-6 md:px-8 py-4">
                <div className="flex flex-wrap items-center gap-3">
                  {/* Add to Calendar */}
                  <button
                    onClick={() => handleAddToGoogleCalendar(event)}
                    className="flex items-center gap-2 bg-warm-900 text-white font-semibold text-sm px-5 py-2.5 rounded-xl hover:bg-warm-700 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-warm-100 text-warm-600 font-semibold text-sm px-5 py-2.5 rounded-xl hover:bg-warm-200 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-warm-100 text-warm-600 font-semibold text-sm px-4 py-2.5 rounded-xl hover:bg-warm-200 transition-colors"
                    title="Copy link"
                  >
